...
if(p_old>pl)//shock
{
  ...
  plfluxfunc_d = pow((Al/(Bl+p_old)),0.5)*(1-((p_old-pl)/(2*(Bl+p_old))));
}
...
if(p_old>pr)
{
  ...
  prfluxfunc_d = pow((Ar/(Br+p_old)),0.5)*(1-((p_old-pr)/(2*(Br+p_old))));
}
else if (p_old<=pr)
{
...
//outputing data
std::ofstream output ("Riemann.dat");
for(int i=0; i<nCells; i++)
{
  double x = x0 + (i+0.5)*dx;
  output<<x<<" "<<u[i][0]<<" "<<u[i][1]<<" "<<u[i][2]<<" "<<u[i][2]/(u[i][0]*(gamma-1))<<std::endl;
}
}
